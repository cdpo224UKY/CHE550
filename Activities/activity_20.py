    ...
    with open('F:/Homo_sapiens.GRCh38.dna.primary_assembly.fa', 'r') as f:
        for line in f:
            ...
            if '>' in line and not any(ident in line for ident in identifiers):
                ...
                # window 1
                AT_count = line[0: 30].count('A')
                AT_count += line[0: 30].count('T')
                if (AT_count / 30.0) > 0.999:
                    sites += 1
                    # print(AT_count / 30.0, sites, line[0: 30])

                # window 1
                AT_count = line[30: 60].count('A')
                AT_count += line[30: 60].count('T')
                if (AT_count / 30.0) > 0.999:
                    sites += 1
                    # print(AT_count / 30.0, sites, line[30: 60])
